...
首次启动时，脚本会从 `authnode.example.json` 创建 `.authnode/config.json`，
并自动初始化本地使用的 `jwt_secret` 和 `admin_token`。不要提交
`.authnode/config.json`。已有的 `authnode.local.json` 仍会兼容读取。
...
- `.authnode/config.json`: 本地 secret 和配置，Git 忽略
